function f = SBM_StatTraceMatchingResult()
...
%sEvalParentFolder = 'E:\SensorMatching\Data\SchoolShuttle\Evaluation20151008_OK';
sEvalParentFolder = 'E:\SensorMatching\Data\SchoolShuttle\Evaluation20160226';

sFileNamePrefix = 'EvRet';
...
for i = 1:length(mCellSelectedFeatures)
    ...
    sEvalResultFile = [sEvalParentFolder '\' sFileNamePrefix];
    
%     for j = 1:length(mSelectedFeatures)
%         sEvalResultFile = [sEvalResultFile '_' num2str(mSelectedFeatures(j))];
%     end
        
    sEvalResultFile = [sEvalResultFile '_' num2str(i)];
    
    sEvalResultFile = [sEvalResultFile '.csv'];
    
    mcellResult = SBM_EvaluteAccuracy(sEvalResultFile, 1);
    
%     mResult = [];
%     mAllAccuracy = mResult{1};
%     mRouteSegAccuracy = mResult{2};
%     mHandAllAccuracy = mResult{3};
%     mHandRouteSegAccuracy = mResult{4};
%     mPocketAllAccuracy = mResult{5};
%     mPocketRouteSegAccuracy = mResult{6};    
