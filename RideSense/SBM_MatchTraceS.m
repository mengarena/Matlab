function [MatchedRouteNo MatchedTraceNo MatchedSegCnt MatchedSegSrc MatchedSegDst nRawRefCombCnt nFeatureMatchingRefCombCnt fMatchedRefPsgDist] = ...
    SBM_MatchTraceS(nPsgRoute, nPsgTrace, nPsgPosition, nPsgSrcSegNo, nPsgDstSegNo, ...
                   mPsgBF, mPsgDF, mSelectedFeatures, ...
...
% This is the version for evaluation which uses all features of all traces
% (even when the bus is at the station)
%
...
% For getting 5 bus lines, this one set (Yellow) is not used
...
sRefPrefixDF = 'Ref_Uniqued_TmCrt_SegDFs_';
sRefStationPrefixDF = 'Ref_Uniqued_TmCrt_StationDFs_';
...
if nUseFilter == 1
    ...
    fTotalMovingDurationErrorThreshold = 150;
    fTotalStopDurationErrorThreshold = 150;
    ...
    for i = 1:nSegCombCnt
        ...
        % Passenger trace and candidate trace should not be large than a
        % threshold
        if nPsgPosition ~= 1  % If phone in hand, don't check here
...
for i=1:nRemainedCombCnt
    ...
    for j = nSrcSegNo:nDstSegNo
        ...
        % Change on 2016/02/26:  Also include the detailed feature of the
        % stations between the segments
        % For two segment t, t+1,  the detailed feature files between them
        % is indexed as t
        sSingleStationDFFile = [sParentFolder '\' sFeatureSubFolder '\' sRefStationPrefixDF num2str(j) '.csv'];
        
        % For the last segment of a trace, there is no station features to be added
        % to the last segment (the corresponding statioin feature file does
        % not exist)
        if exist(sSingleStationDFFile, 'file')
            mSingleStationDF = load(sSingleStationDFFile);
            mRefDF = [mRefDF; mSingleStationDF];            
        end
    end
