function f = SBM_GetWinGyroFeaturePCA(mWinGyroRaw, nIdxDataType, fPCAStat, pcaCompIdx)
% This function calculate the Gyro feature within a window
%
% Different from [SBM_GetWinGyroFeature], this function applys PCA on
...
for i = 2:nRow
    if mWinGyro(i,1) == mWinGyro(i-1,1) || mWinGyro(i,1) == fPreviousRawTm
        fPreviousRawTm = mWinGyro(i,1);
        mWinGyro(i,1) = mWinGyro(i-1,1) + fMinTimeGap;
    end
end

mWinPureGyro = mWinGyro(:,nIdxDataType+1:nIdxDataType+3);

[coeff, score, latent, ts, explained] = pca(mWinPureGyro);

fprintf(fPCAStat, '2, %f, %f, %f, %f, %f, %f,\n', explained(1), explained(2), explained(3), latent(1), latent(2), latent(3));
...
% Get magnitude
%mMagnitude = sqrt(sum(power(mWinGyro(:,nIdxDataType+1:nIdxDataType+3),2),2));
...
fMagSMA = (sum(abs(mWinGyro(:,nIdxDataType+1))) + sum(abs(mWinGyro(:,nIdxDataType+2))) + sum(abs(mWinGyro(:,nIdxDataType+3))))/nRow;   % Normalized Signal Magnitude Area 
...
fMeanX = mean(mWinGyro(:,nIdxDataType+1));
fMeanY = mean(mWinGyro(:,nIdxDataType+2));
fMeanZ = mean(mWinGyro(:,nIdxDataType+3));
f3AxisAAD = mean(abs(mWinGyro(:,nIdxDataType+1)-fMeanX) + abs(mWinGyro(:,nIdxDataType+2)-fMeanY) + abs(mWinGyro(:,nIdxDataType+3)-fMeanZ));   % Average Absolute Difference
...
fInterval = (mWinGyro(nRow,1) - mWinGyro(1,1))/(nSampleCnt-1);
xx = mWinGyro(1,1):fInterval:mWinGyro(nRow,1);
yy = spline(mWinGyro(:,1),mMagnitude,xx);    % Interpolate
