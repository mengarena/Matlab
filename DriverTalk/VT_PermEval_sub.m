function f = VT_PermEval_sub(nHighWay, mTrace, mTargetList, bDense, nRange, nImgSize, nImgSmallPktSize, fImgMsgRcvTimeGap, fMapMsgRcvTimeGap, nFixedImgCnt, fEndTime)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%[Begin] Here new added
[nTargetRow nTargetCol] = size(mTargetList);
nTargetListRowIdx = 1;

curTargetSender = mTargetList(nTargetListRowIdx, 1);
curTargetSenderTime = mTargetList(nTargetListRowIdx, 2);
curTargetImgCount = mTargetList(nTargetListRowIdx, 3);
curTargetCount = mTargetList(nTargetListRowIdx, 4);
%%[End]
...
while i <= nRow
    ...
    if nEventTypeS == 1    % Send event
        %%[Begin] Here new added
        if curTargetCount == 0
            i = i + nSmallPktCount*curTargetImgCount;
            
            nTargetListRowIdx = nTargetListRowIdx + 1;
            curTargetSender = mTargetList(nTargetListRowIdx, 1);
            curTargetSenderTime = mTargetList(nTargetListRowIdx, 2);
            curTargetImgCount = mTargetList(nTargetListRowIdx, 3);
            curTargetCount = mTargetList(nTargetListRowIdx, 4);
    ...
    %            nSourceIdT = mTrace(j, 5);
            ...
            for j=i+1:nRow
                ...
                nIsTargetNode = 0;
                for t=1:curTargetCount
                    if mTargetList(nTargetListRowIdx, t+4) == nNodeIdT
                        nIsTargetNode = 1;
                        break;
                    end
                end
                
                if nIsTargetNode == 0  %This receiver is not the target receiver
                    continue;
                end
                %%[End]
...
avgMapMsgTransTime = 0;
stdMapMsgTransTime = 0;
maxMapMsgTransTime = 0;
minMapMsgTransTime = 0;
